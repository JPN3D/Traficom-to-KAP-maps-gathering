...
| `make_catalog.py` | Luo luettelotiedoston automaattisesti WMTS-tason maantieteellisen rajauksen perusteella — hyödyllinen kun valmiita lehden kulmakoordinaatteja ei ole saatavilla |
...
## Luettelotiedoston automaattinen luominen (`make_catalog.py`)

Jos valmiita lehden kulmakoordinaatteja ei ole saatavilla — esimerkiksi
kun käytetään tasoa kuten `Traficom:Yleiskartat 250k public`, jolla ei
ole virallista lehtijakoa — `make_catalog.py` voi luoda luettelon
automaattisesti jakamalla tason maantieteellisen rajauksen säännölliseksi
ruudukoksi.
...
Generoitu luettelo ajetaan samoin kuin mikä tahansa muu luettelo:
...
| Valitsin | Kuvaus |
|---|---|
| `--layer NIMI` | Tarkka WMTS-tason tunniste (aja `py list_layers.py` löytääksesi sen) |
| `--dlat N` | Lehden korkeus leveysasteina (oletus 1.5) |
| `--dlon N` | Lehden leveys pituusasteina (oletus 3.0) |
| `--overlap N` | Vierekkäisten lehtien päällekkäisyys (oletus 0.05 = 5 %) |
| `--prefix TEKSTI` | Lehtitunnuksen etuliite, esim. `YK250` → tunnukset kuten `YK250_0001` |
| `--zoom N` | Valinnainen: tulostaa arvioidun pikselikoon ja laattamäärän per lehti tällä zoom-tasolla, jotta voit tarkistaa luvut ennen täyttä latausta |
| `--out TIEDOSTO` | Luotavan luettelotiedoston nimi |

**Vinkki:** testaa ensin yhdellä lehdellä `--only`-valitsimella ennen koko
luettelon ajamista, sillä yleiskarttatasot hienoilla zoom-tasoilla voivat
tuottaa hyvin suuria rastereita:
